...
- Les caractéristiques doivent être en français et pertinentes (2-4 max)`;
      ...
      const clustering: ClusteringResult = {
        ...
        characteristics: Array.isArray(parsed.clustering?.characteristics) ? parsed.clustering.characteristics.slice(0, 4) : ["Profil équilibré"],
        usingGroq: true,
        model: modelName,
      };
   ...
   */
  private fallbackAnalysis(features: ProcessedFeatures, totalPlaytime: number): { classification: ClassificationResult; clustering: ClusteringResult } {
    const threshold = 500;
